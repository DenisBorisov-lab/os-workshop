...
class Controller {
private:
    std::unordered_set<int> node_ids_; // Идентификаторы узлов
    std::list<Message> pending_messages_; // Очередь сообщений ожидающих оправки
    std::list<Node> children_; // Список дочерних узлов
    std::chrono::milliseconds heartbeat = std::chrono::milliseconds::zero(); // Период времени между "пульсацией вычислительных узлов"
    std::map<int, std::chrono::system_clock::time_point> beat_tracker; // сопоставляет идентификаторы узлов с последним времени "пульса"
    ...
    // Проверяет ожидающие сообщения и удаляет те, у которых истекло время исполнения
    ...
    // Удаляет сообщение из очереди сообщений
    ...
    // Добавляет сообщение в список ожидания и рассылает его всем дочерним узлам
    ...
    // Обрабатывает команду для создания узла
    ...
    // Исполняет ping для проверки жизнеспособности узла
    ...
    // Настраивает интервал пульса узлов
    ...
    // Проверяет не отсутствует ли пульс у узлов
    void check_beats() {
        for (auto& [key, value] : beat_tracker) {
